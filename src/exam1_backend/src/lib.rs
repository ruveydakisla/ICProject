...
struct Proposal{
    ...
    pass:u32,
    is_active:bool,
    voted:Vec<candid::Pricipal>
    owner:candid::Pricipal
}
...
thread_local! {
    ...
    static PROPOSAL_MAP:ReffCell<StableBTreeMap<u64,Proposal,Memory>> = ReffCell::new(StableBTreeMap::init(MEMORY_MANAGER.with(|m|m.borrow.get(MemoryId::new(0)))));
...
fn edit_proposal(key: u64, proposal: CreateProposal) -> Result<(), VoteError> {
    ...
    match old_proposal_opt{
        Some(old_proposal){
            ...
            None=Zreturn Err(VoteError::NoSuchProposal),
        }
